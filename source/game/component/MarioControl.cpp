...
{
	this->action = MarioAction::Walking;
}
...
bool	MarioControl::actGroupMoving()
{
	...
	/* clang-format off */
	switch (this->action) {
		case MarioAction::Walking:			this->actWalking();					break;
		// case MarioAction::HoldWalking:		act_hold_walking(m);             break;
		...
		// case MarioAction::Braking:			this->actBraking();					break;
		...
		// case MarioAction::Decelerating:		this->actDecelerating();			break;
...
bool	MarioControl::actWalking()
{
	...
	//     return begin_braking_action(m);
	// }
	// if (this->intended_mag == 0) // no dpad input
	// 	this->beginBraking();
...
void	MarioControl::setMarioAction(MarioAction action)
{
	switch ((u32)action >> 8)
	{
		// case 0: // Stationary
		...
		// 	break;
		case 1: // Moving
			this->setMarioActionMoving(action);
			break;
		case 2: // Airborne
			break;
		case 3: // Submerged
		...
		case 5: // Automatic
			break;
		case 6: // Object Action
			break;
	}
}

void	MarioControl::setMarioActionMoving(MarioAction action)
{
	i32 mag = std::min(this->intended_mag, 8 * 256);

	switch (action)
	{
	case MarioAction::Walking:
		// if (floorClass != SUFACE_CLASS_VERY_SLIPPERY)
		if (0 <= this->forward_vel && this->forward_vel < mag)
			this->forward_vel = mag;
		break;
	case MarioAction::HoldWalking:
		if (0 <= this->forward_vel && this->forward_vel < (mag >> 1))
			this->forward_vel = mag >> 1;
		break;
	case MarioAction::BeginSliding:

		break;
	}
}

void	MarioControl::beginBraking()
{
	// mario_drop_held_object(m);

    // if (this->actionState == 1)
	// {
	// 	this->yaw = this->action_argument;
    //     // m->faceAngle[1] = m->actionArg;
    //     return set_mario_action(m, ACT_STANDING_AGAINST_WALL, 0);
    // }

    if (this->forward_vel >= 16 * 256)//  && m->floor->normal.y >= 0.17364818f) {
	{
		this->setMarioAction(MarioAction::Braking);
		return;
    }
	this->setMarioAction(MarioAction::Decelerating);
}

	...
	void	rasterize(u32 a, u32 b) {
...

		{
			...
			}
			{
				...
				u32 inv_a_dy = reciprocal((u32)std::max(a_h, 1)) >> 16;
				u32 inv_b_dy = reciprocal((u32)std::max(b_h, 1)) >> 16;
				a_dx = (i32(va1 & 0x0000FFFF) - i32(va0 & 0x0000FFFF)) * inv_a_dy;
				b_dx = (i32(va2 & 0x0000FFFF) - i32(va0 & 0x0000FFFF)) * inv_b_dy;
				a_du = (du01 * inv_a_dy);
				b_du = (du02 * inv_b_dy);
				a_dv = (dv01 * inv_a_dy);
				b_dv = (dv02 * inv_b_dy);
